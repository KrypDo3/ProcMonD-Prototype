...
from procmond import daemon
from procmond.core.detectors import detect_process_without_exe
...
def create_test_db(path: str) -> sqlite3.Connection:
    ...
    cur = conn.cursor()
    cur.execute(
        "CREATE TABLE processes (id INTEGER, ppid INTEGER, updated_at DATETIME, name VARCHAR, "
        'path VARCHAR, valid BIT, "hash" VARCHAR, accessible BIT, file_exists BIT, '
...
def test_detect_process_without_exe(monkeypatch, tmp_path) -> None:
    db = tmp_path / "test.db"
    ...
    monkeypatch.setattr(daemon.config, "database_path", str(db))
